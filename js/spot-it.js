document.addEventListener('DOMContentLoaded', () => {
            ...
            diffAreas: [
                {
                    ...
                    circles: [
                        { centerX: 72, centerY: 84, radius: 20 }
                    ]
                },
                {
                    id: 'diff2',
    ...
    function handleClick(event) {
        if (foundDifferences === totalDifferences) {
            messageDisplay.textContent = '모든 틀린 그림을 찾았습니다! 다시 시작 버튼을 눌러주세요.';
            return;
        }
        ...
        if (foundDiffIndex !== -1) {
            ...
            highlightDifference(foundDiff.displayCircle, displayedWidth, displayedHeight, naturalWidth, naturalHeight);
            found = true;
        } else {
            messageDisplay.textContent = '아쉽지만 틀린 부분이 아닙니다. 다시 시도해보세요!';
            // 잠시 메시지 표시 후 초기화
            setTimeout(() => {
                messageDisplay.textContent = `찾은 틀린 그림: ${foundDifferences} / ${totalDifferences}`;
            }, 1000);
    ...
    function highlightDifference(circleInfo, displayedWidth, displayedHeight, naturalWidth, naturalHeight) {
